    ...
    try:
        ...
        if proxy:
            try:
                ...
                logger.info(f"[PROXY] Используется: {proxy}")
            except Exception as e:
                logger.error(f"[PROXY ERROR] {e}")
                session = AiohttpSession()
        else:
            logger.info("[PROXY] Бот запущен без прокси")
            session = AiohttpSession()

        return Bot(token=TELEGRAM_TOKEN, session=session)

    except Exception as e:
        logger.error(f"[BOT CREATE ERROR] {e}")
        return Bot(token=TELEGRAM_TOKEN, session=AiohttpSession())
...
async def main():
    ...
    register_twitch_api_handler(dp)   # ✅ ВОТ ТУТ ПРАВИЛЬНО

    # ---------------- watchers ----------------
    asyncio.create_task(watch_streamers_file(CHECK_INTERVAL))
